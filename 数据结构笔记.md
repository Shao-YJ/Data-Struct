...
- 设无向图有n个顶点vi，m条边，vi的度为TD(vi)，则![1666145521004](F:\ProgramData\自动缓存文件\TyporaCache\1666145521004.png)
...
问题描述：求图中**任意一对顶点**之间的**最短路径**。

Floyd算法的基本思想：

- 从vi到vj的所有可能存在的路径中，选出一条加权长度最短的路径
- =>若<vi,vj>存在，则存在路径{vi, vj}——路径中不含其它顶点
- =>若<vi, u0>,<u0, vj>存在，则存在路径{vi, u0, vj}; ——路径中还包含u0顶点
- =>若<ui, u0>，<u0, u1>，<u1,vj>存在，则存在路径{vi, u0, u1, vj}；——路径中还包含u0和u1两个顶点
- =>依次类推。
...
单源最短路径：从指定顶点v, 要求在图G中找到从顶点v到其它各顶点的最短路径——Dijkstra算法	时间复杂度为O(n2)。

求图G中任意一对顶点之间的最短路径——Floyd算法时间	复杂度为O(n3)。

## 4.6有向无环图

**有向无环图**：一个没有回路 (即无环)的有向图。主要用于研究工程项目的工序问题、工程时间进度问题等。

![1667101314336](F:\ProgramData\自动缓存文件\TyporaCache\1667101314336.png)
...
1. 如果从有向图G的某个顶点v出发，进行**深度优先搜索遍历**时产生顶点u**到顶点v的回边**，则图G存在包含顶点u和顶点v的环。
2. 对有向图G进行**拓扑排序**。如果**所有顶点**都包含在“**拓扑序列**”中，则图G不存在环。

## 4.7拓扑排序

**拓扑序列**：假设对有向图进行操作：按照有向图的弧 (次序关系)，将图中顶点排成一个线性序列(**对于图中没有限定次序关系的顶点, 可以人为地添加上任意的次序关系**)，由此得到的顶点序列称为**拓扑序列**。

例:

- 在由顶点v1，v2，v3和v4组成的有向图中，可以得到两个拓扑序列：v1，v2,v3,v4和v1,v3,v2,v4。

![1667112961612](F:\ProgramData\自动缓存文件\TyporaCache\1667112961612.png)

- 而在下面的有向图中，拓扑序列唯一：v1,v3,v2,v4

![1667113012245](F:\ProgramData\自动缓存文件\TyporaCache\1667113012245.png)

- 在下面的有向图中，不能求得拓扑序列——因为图中存在一个环 {v2,v4,v3}

![1667113083767](F:\ProgramData\自动缓存文件\TyporaCache\1667113083767.png)

**拓扑排序的一般操作方法**：

1. 从有向图中选取一个**入度为0**的顶点，输出它;
2. 从有向图中删去**该顶点**以及**所有以它为尾的弧**;
3. 重复上述两步，直至**图空**或者**找不到入度为0的顶点**为止。例如，
...
**AOE网**（Activity on Edge)：AOE网是一个**带权有向无环图**。其中，**顶点表示事件**，用**弧表示活动**，**权表示活动持续的时间**。例如，

 ![img](https://img-blog.csdn.net/20180719142430705?watermark/2/text/aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM3NjE4Nzk3/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70) 
...
**关键活动**: **关键路径上的活动**称为关键活动。关键活动是**影响整个工程的关键**。

**事件vi的最早发生时间**: **设v0是起点**，**从v0到vi的最长路径长度**称为事件vi的最早发生时间，即是以vi为尾的所有活动的最早发生时间。

 若活动ai是弧<j,k>，持续时间是dut(<j,k>)，设：

- e(i)：表示活动ai的**最早开始时间**；
- l(i)：在**不影响进度的前提**下，表示活动ai的**最晚开始时间**； 则**l(i)-e(i)表示活动ai的时间余量**，若**l(i)-e(i)=0**，**表示活动ai是关键活动**。
- ve(i)：表示**事件vi的最早发生时间**，即从起点到顶点vi的最长路径长度； 
- vl(i)：表示**事件vi的最晚发生时间**。

则有以下关系：

![1667181228780](F:\ProgramData\自动缓存文件\TyporaCache\1667181228780.png)

![1667181570318](F:\ProgramData\自动缓存文件\TyporaCache\1667181570318.png)
...
![1667182236381](F:\ProgramData\自动缓存文件\TyporaCache\1667182236381.png)
...
1. 利用**拓扑排序**求出AOE网的一个**拓扑序列**； 
2. 从拓扑排序的序列的**第一个顶点(源点)**开始，按**拓扑顺序**依次计算**每个事件的最早发生时间ve(i)** ； 
3. 从拓扑排序的序列的**最后一个顶点(汇点)**开始，按**逆拓扑顺序**依次计算**每个事件的最晚发生时间vl(i)** ；

**关键路径**：从源点到汇点的**最长路径**。

例如，v1→v2→v5→v8→v9是关键路径。

![1667182612138](F:\ProgramData\自动缓存文件\TyporaCache\1667182612138.png)
...
**平均查找长度**(Average Search Length)
 ...
 ASL=![1667202538432](F:\ProgramData\自动缓存文件\TyporaCache\1667202538432.png)

其中, **n为数据表长度**, **Pi为查找数据表中第i个记录的概率**，![1667202569855](F:\ProgramData\自动缓存文件\TyporaCache\1667202569855.png)，Ci为找到第i个记录时已经进行比较的次数。
...
顺序查找过程：从数据表中的**最后一个记录(或第一个记录)**开始，逐个比较关键字和给定值，并判断查找是否成功。

**例**：在数据表L[1..n]中，顺序查找关键字=key的记录。若找到，返回该记录在数据表中的位置，否则返回0。

算法分析：从n到1，逐个比较**关键字=key**，并根据比较情况输出查找结果。
...
![1667202981165](F:\ProgramData\自动缓存文件\TyporaCache\1667202981165.png)

在等概率查找的情况下，

![1667202991455](F:\ProgramData\自动缓存文件\TyporaCache\1667202991455.png)

即顺序表查找的平均查找长度为:

![1667203051528](F:\ProgramData\自动缓存文件\TyporaCache\1667203051528.png)
...
将数据按照**一定原则**存入数据表，使数据表划分为若干个逻辑子表。
...
分块查找：先用**顺序查找法**在**索引表**中查找**索引记录**，再依据该记录值在数据表中的相应**逻辑子表查找所需要的记录**。
...
![1667204883383](F:\ProgramData\自动缓存文件\TyporaCache\1667204883383.png)
...
对于每一棵二叉排序树，均可按照**平均查找长度**的定义来计算它的ASL值。

由n个关键字值构造所得的不同形态的二叉排序树, 其平均查找长度不一定相同，甚至可能差别很大。

![1667219284096](F:\ProgramData\自动缓存文件\TyporaCache\1667219284096.png)

![1667219297324](F:\ProgramData\自动缓存文件\TyporaCache\1667219297324.png)
...
![1667266939793](F:\ProgramData\自动缓存文件\TyporaCache\1667266939793.png)

![1667266952910](F:\ProgramData\自动缓存文件\TyporaCache\1667266952910.png)
...
在构造二叉排序树的过程中，如果出现不平衡结点，需采用**平衡方法**将二叉树调整成平衡二叉树，其要点是：将**关键字中间值**的结点作为**根结点**，再重新连接相关结点，构成平衡二叉树。

**不平衡二叉树的基本形态**：

![1667267314958](F:\ProgramData\自动缓存文件\TyporaCache\1667267314958.png)

**二叉排序树的基本平衡方法**：

1. ![1667267372351](F:\ProgramData\自动缓存文件\TyporaCache\1667267372351.png)

2. ![1667267513918](F:\ProgramData\自动缓存文件\TyporaCache\1667267513918.png)

3. ![1667269201942](F:\ProgramData\自动缓存文件\TyporaCache\1667269201942.png)

4. ![1667269231206](F:\ProgramData\自动缓存文件\TyporaCache\1667269231206.png)

5. ![1667269734127](F:\ProgramData\自动缓存文件\TyporaCache\1667269734127.png)

   ![1667269792845](F:\ProgramData\自动缓存文件\TyporaCache\1667269792845.png)

6. ![1667269824164](F:\ProgramData\自动缓存文件\TyporaCache\1667269824164.png)

   ![1667269853774](F:\ProgramData\自动缓存文件\TyporaCache\1667269853774.png)

7. ![1667269933204](F:\ProgramData\自动缓存文件\TyporaCache\1667269933204.png)

   ![1667270011508](F:\ProgramData\自动缓存文件\TyporaCache\1667270011508.png)

8. ![1667270079047](F:\ProgramData\自动缓存文件\TyporaCache\1667270079047.png)

   ![1667270094844](F:\ProgramData\自动缓存文件\TyporaCache\1667270094844.png)
...
哈希表主要包括**构造哈希函数**、**处理冲突**和**哈希表查找**等内容。
...
哈希表举例：设线性表A= { 18, 75, 43, 55, 60, 46 }，哈希函数 H(key)=key%m。如果取m=6 (最小存储空间)，则H(18)=0，H(75)=3，H(43)=1，H(55)=5，H(60)=0，H(46)=4；这时，**H(60)=0和H(18)=0发生冲突**，必须**重新分配哈希地址(即需要处理冲突)**。 
...
取关键字的线性函数作为哈希函数：H(key) = a × key + b

其中，a和b为常数。此方法适合于：地址集合的大小≥关键字集合的大小。

例如，关键字集={11, 22, 33, 44, 55}，m=5

(地址集合的大小=关键字集合的大小)， 存储地址依次是0, 1, 2, 3和4。

=> 取哈希函数H(key) = key/11-1，即取a=1/11，b= –1。

#### **除留余数法**

设哈希表表长为m，取哈希函数为H(key) = key % p，p≤m。

要点：选择一个合适的p值，使H(key)的同义词尽可能少(尽可能少冲突)。一般情况下，可以选择**p为素数**。
...
其中，H0 = H(key)，Hi =( H0+di )%m，i=1，2，…，s (di称为增量)

1. 线性探测再散列：di = c × i，其中c为常数，i =1,…,m-1
2. 二次探测再散列：di = 1^2，-1^2，2^2，-2^2，……

注意：增量di应具有“完备性”：s个Hi均不相同，且都是哈希表的有效地址。

**例**：表长m=11，关键字集={ 19, 1, 23, 14, 55, 68, 11, 82, 36 }。

设定哈希函数H(key) = key % 11，采用线性探测再散列处理：di = i，则
...
取哈希函数H(key)=key%5+1，则哈希地址={5, 4, 1, 2, 3, 4}

由链地址法产生的哈希表

![1667274218774](F:\ProgramData\自动缓存文件\TyporaCache\1667274218774.png)
...
- 否则，按原**处理冲突方法**求下一个哈希地址Hi，**直至Hash[Hi].key=k0**，或者**Hash[Hi]=NULL**为止。
...
从查找过程得知，哈希表的**平均查找长度实际上并不等于1**。

决定哈希表平均查找长度ASL的因素：**处理冲突的方法**和**装填因子**。

例如，对于表长m=6，关键字集={ 19，23， 55， 11， 47， 38 }，如果采用链地址法，ASL=(1×5+2×1)/6 >1

装填因子定义：![1667275390167](F:\ProgramData\自动缓存文件\TyporaCache\1667275390167.png)

一般情况下，a越小，发生冲突的可能性越小；a越大，填记录就越容易发生冲突。  

哈希表的**一个特点**：

用哈希表构造列表时，可以选择一个适当的装填因子α，使得平均查找长度限定在某个范围内。一般情况下，哈希表的平均查找长度是α的函数。如利用线性探测再散列处理冲突法的平均查找长度![1667275490769](F:\ProgramData\自动缓存文件\TyporaCache\1667275490769.png)

**例题**：已知列表L中的1000个关键字各不相同。请给出列表L的一个哈希表设计方案，要求它在等概率情况下，查找成功时的平均查找长度不超过3。

1. 选择处理冲突的方法，并求出α——“线性探测再散列”处理冲突法。

2. 由于要求平均查找长度≤3，则![1667275682821](F:\ProgramData\自动缓存文件\TyporaCache\1667275682821.png)得到α≤0.8

3. 确定哈希表的长度m，一般取m≥k/α中的最小素数。

   ∵ k/a=1000/0.8=1250=> m=1259 (根据给定的素数表找出**满足要求的最小质数**)。

# 6排序 Sort

**基本概念**

排序定义：假设含n个元素的数据表为{r1, r2, …, rn}，试确定1, 2, …, n的一种排列p1, p2, …, pn，使数据表满足非递减关系：![1667276172948](F:\ProgramData\自动缓存文件\TyporaCache\1667276172948.png)

即数据表![1667276197362](F:\ProgramData\自动缓存文件\TyporaCache\1667276197362.png)递增有序。

排序的**稳定性**：对于数据表{r1, r2, …, rn}，当ri=rj且i<j时,如果排序前ri领先于rj，排序后ri仍然领先于rj，则称该排序方法是稳定的；否则称该排序方法是不稳定的。![1667276435614](F:\ProgramData\自动缓存文件\TyporaCache\1667276435614.png)

排序的**分类**：

- 物理排序：根据**关键字值**在数据表中**重新排列**记录的**存储位置**。

- 索引排序：根据关键字值**建立索引表**，在**输出数据表**的记录时，**由索引表描述输出顺序**(索引表当作静态链表用)。
- 比较式排序：根据数据表中两个元素值的大小，确定这两个元素的存储位置。
  - 两种基本操作：
    1. 比较两个关键字的大小
    2. 交换两个记录的位置
  - 两类数据区：![1667276643145](F:\ProgramData\自动缓存文件\TyporaCache\1667276643145.png)
...
- 比较次数：![1667370610911](F:\ProgramData\自动缓存文件\TyporaCache\1667370610911.png)
- 移动次数：0

最坏情况(关键字在数据表中逆序有序)：

- 比较次数：![1667370820368](F:\ProgramData\自动缓存文件\TyporaCache\1667370820368.png)
- 移动次数：![1667371058776](F:\ProgramData\自动缓存文件\TyporaCache\1667371058776.png)
...
- 将整个数据表分成**m个子序列**，对**每个子序列**分别进行**插入排序**。
...
最坏情况(关键字在数据表中递减有序)：需进行n-1趟冒泡。

- 比较的次数：![1667443148734](F:\ProgramData\自动缓存文件\TyporaCache\1667443148734.png)
- 移动的次数：![1667443154536](F:\ProgramData\自动缓存文件\TyporaCache\1667443154536.png)

### 地精排序

地精排序：单重循环的排序算法

例 问题：数组L[n]存放n个整数，用**一重循环**将L[n]中的整数从小到大排序。
...
基本思想：将两个(2路)或两个以上的**有序(子)序列**合并为一个有序序列。
...
- 通过一趟快速排序将待排数据分割成**独立的左中右三个部分**，其中，**中部分只含一个数**，**左边部分**的所有**关键字都比这个数小**，**右边部分**的所有**关键字都比这个数大**。

- 然后再按此方法对这两部分记录分别进行快速排序，直到所有记录整理成有序序列。 

选取一个数据L[i]，以它的关键字值key作为基准元素(枢轴)进行划分：将顺序表L[s..t]划分成三个部分L[s..i-1]，L[i]和L[i+1..t]：

![1667447553666](F:\ProgramData\自动缓存文件\TyporaCache\1667447553666.png)
...
![1667458613231](F:\ProgramData\自动缓存文件\TyporaCache\1667458613231.png)

若待排记录的**初始状态按关键字有序**，快速排序将蜕化为**冒泡排序**，其时间复杂度为O(n^2)。

为避免出现这种情况，需在进行一次划分之前，进行“预处理” 。例如，先对L(s).key, L(t).key和L[(s+t)/2].key进行相互比较，然后取关键字为 “**三者之中**”的记录作为枢轴记录。
...
![1667614879326](F:\ProgramData\自动缓存文件\TyporaCache\1667614879326.png)
...
![1667615080945](F:\ProgramData\自动缓存文件\TyporaCache\1667615080945.png)

![1667615085380](F:\ProgramData\自动缓存文件\TyporaCache\1667615085380.png)
...
- 设r[s..t]中的关键字除r[s]外均满足小顶堆的特征(二叉树的基本形状满足要求)。
  ...
  ![1667617991752](F:\ProgramData\自动缓存文件\TyporaCache\1667617991752.png)
...
计数排序是一种计算式算法，要求数据元素为**整型**。

设数据元素(整数)的**个数为n**，数据元素**最大值和最小值的差值为m-1**。

如果m<log2n，计数排序算法的时间复杂度小于比较式排序算法。
...
1. 找出数组r中最小值m1和最大值m2;
2. 统计数组r中每个值为i的整数出现的次数，存入数组s的s[i-m1]项；
3. 对i=2, …, m2-m1+1，计算s[i]+=s[i-1]；
4. 填充目标数组：将每个值为i的整数存放在数组的L[s[i]]项，并执行s[i]-1。

![1667631978281](F:\ProgramData\自动缓存文件\TyporaCache\1667631978281.png)
...
树形选择排序又称锦标赛排序，是一种按照**锦标赛**的思想进行选择排序的方法。首先对**n个记录的关键字进行两两比较**，然后在**n/2个较小者**之间再进行**两两比较**，如此重复，直至选出最小的记录为止。
...
完全二叉树的**前h-1层**是1棵**满二叉树**，共有2^(h-1-1个结点。n个数据存放在完全二叉树的底层，编号从2^(h-1)到2^(h-1)+n-1。
...
1. 初始化 (时间复杂度为O(n))
   - 计算满二叉树的高度 h = [log2] （向上取整）
   - 满二叉树的结点值都置成 ∞ (令k=2^h-1)：for (i=1; i<=k; i++) L[i]= ∞;
   - n个数据存放在底层：L[k+1..k+n]。
   - 注意，还需设置L[k+n+1]= ∞。
...
void TreeSort(T L[],int n) {
	...
	for (i = h; i >= 1; i--) {
		...
		for (j = start; j < end && j <= k + n; j += 2) 
			tree[j / 2] = tree[j] < tree[j + 1] ? tree[j] : tree[j + 1];//将2n与2n+1中较小值赋给父结点
	}
...
假如**多关键字**的记录序列中，每个关键字的**取值范围相同**(如十进制数)，则按 **LSD (Least Significant Digit first，最低位优先法)** 法进行排序时，可以采用 “**分配-收集**” 法，该方法不需要进行关键字之间的比较。

基数排序方法的基本思路是，将单关键字看成是由**多个数位** (或多个字符)构成的**多关键字**，并采用 “分配-收集”的方法进行排序。
...
1. 将**待排记录**建成一个链表;
2. 分配时，将**当前关键字位值相同的记录**分配到**同一个链队列**中；
3. 收集时，按**当前关键字位取值从小到大顺序**将**各链队列链成一个链表**；
4. 对每个关键字位**重复(2)和(3)两步**。
...
其中：分配算法时间复杂度为O(n)，收集为O(r)，r=Radix为“基数”，k为“分配-收集”的趟数

## 6.8桶排序(先不学了)

定义：假设输入是由随机过程产生的**[0, 1)区间上均匀分布的实数**。将区间[0, 1)划分为**n个大小相等的桶**(子区间)，每桶大小为**1/n**。将n个元素分配到这些桶中，对桶中元素进行排序，然后依次连接桶。